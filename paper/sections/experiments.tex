...
Unlike our earlier evaluation (which assigned 0 to all baselines), the fair evaluation reveals that ToG achieves BroadFaith@10 of 0.94 and Coverage of 1.00---it does explore and output KG paths. However, ToG's \emph{strict} faithfulness is only 0.05, meaning most hypotheses are only partially supported by their paths. KG-SCoRE achieves StrictFaith@10 of 0.10 (double ToG's) and BroadFaith@10 of 0.99. Vanilla LLM has near-zero coverage (0.11), confirming it generates without KG grounding. BM25-RAG has moderate coverage (0.57) but near-zero faith, indicating retrieved text snippets rarely align with KG paths. (RoG fair faithfulness results were not completed due to compute constraints; we report its recall in Table~\ref{tab:main}.)

\paragraph{Finding 2: No significant recall difference from RoG.}
On 20 pairs, KG-SCoRE (0.25) and Vanilla LLM (0.25) are identical, with RoG slightly higher (0.30). On the 10-pair subset (multi-seed, Table~\ref{tab:multiseed}), KG-SCoRE achieves 0.53$\pm$0.05 vs.\ RoG's 0.57$\pm$0.05. McNemar test shows no significant difference ($p{=}0.80$). Note: $p{=}0.80$ indicates failure to detect a difference, not proof of equivalence.

\paragraph{Statistical significance.}
McNemar tests on 10-pair multi-seed ($n{=}30$) confirm KG-SCoRE significantly outperforms Vanilla LLM ($p{=}0.0005$), BM25-RAG ($p{=}0.0001$), and ToG ($p{=}0.010$), but not RoG ($p{=}0.80$).
...
\begin{table}[tb]
  ...
  \begin{tabular}{lcc}
    ...
    \midrule
    Vanilla LLM & 0.13 $\pm$ 0.09 & [0.20, 0.00, 0.20] \\
    BM25-RAG & 0.13 $\pm$ 0.05 & [0.20, 0.10, 0.10] \\
    ToG & 0.20 $\pm$ 0.00 & [0.20, 0.20, 0.20] \\
    \textbf{RoG} & \textbf{0.57 $\pm$ 0.05} & [0.60, 0.50, 0.60] \\
    \textbf{KG-SCoRE} & \textbf{0.53 $\pm$ 0.05} & [0.50, 0.60, 0.50] \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[tb]
  \centering
  \caption{KG cleaning evaluation (200 triples, 100 noise + 100 clean).}
  \label{tab:cleaning}
  \small
  \begin{tabular}{lcccc}
    \toprule
...
\noindent\textit{Note: The KG cleaning evaluation was in progress at submission time. Results will be reported in the camera-ready version. The experimental design: for each triple, the LLM verifies whether it is supported/contradicted/unverifiable; we compute precision (fraction of LLM-flagged triples that are actually noise), recall (fraction of true noise detected), and F1.}

\subsection{KGQA Generalization}
\label{sec:exp:kgqa}

We construct 9 multi-hop QA pairs from the KG. All methods achieve EM=0.00 and F1=0.00 because synthetic entity names (e.g., ``chemical\_42'') are absent from the LLM's parametric knowledge, making exact match impossible for any method. KG-SCoRE achieves BroadFaith@10 of 0.89, but this only demonstrates that it can generate path-consistent (but incorrect) answers. This experiment highlights that \emph{faithfulness and correctness are decoupled}: a method can be perfectly faithful to a path while producing a wrong answer. We acknowledge this experiment does not demonstrate task-level generalization and plan to evaluate on standard benchmarks (WebQSP, MetaQA) in future work.

\subsection{Discussion}
The results paint a nuanced picture. Under fair evaluation, KG-SCoRE's faithfulness advantage is real but smaller than initially suggested: ToG also achieves high BroadFaith (0.94) and Coverage (1.00), but its StrictFaith is low (0.05). KG-SCoRE doubles StrictFaith (0.10) while maintaining competitive recall. The ComplEx component is counterproductive on noisy KGs---a key design finding. The faithfulness metric's fine-grained labels are unreliable ($\kappa{\approx}0$), but the aggregate BroadFaith score is stable across judge types (0.49--0.50). These findings suggest that LLM-based verification is a viable approach for producing traceable hypotheses, but the faithfulness metric requires refinement (stricter labels, trained entailment models).
