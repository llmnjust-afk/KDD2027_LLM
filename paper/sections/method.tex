...
For each query, Module~A verifies $K$ triples (default $K{=}40$), requiring $K$ LLM calls. Module~C requires 1 LLM call for generation, and Module~D requires $K$ LLM calls for faithfulness checking. Total: $\sim$$2K{+}1$ LLM calls per query ($\sim$81 calls with $K{=}40$).
